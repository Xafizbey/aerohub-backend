...
from app.schemas.analytics import MovieStats, MusicStats, TrackPlay, TrackView
...
async def get_movie_stats(db: AsyncSession, limit: int = 10) -> list[MovieStats]:
    stmt = (
        select(
            ...
            func.avg(ViewHistory.completed.cast("int")).label("completion_rate"),
        )
...
async def get_music_stats(db: AsyncSession, limit: int = 10) -> list[MusicStats]:
    stmt = (
        select(
            ...
            func.avg(PlayHistory.completed.cast("int")).label("completion_rate"),
        )
